...
public class MainScreen {
    ...
    private final List<NumberText> numberFields = new ArrayList<>();
    ...
    private JPanel generateSection(final List<Space> spaces) {
        ...
        fields.forEach(t -> {
            notifierService.subscribe(CLEAR_SPACE, t);
            numberFields.add(t);
        });
    ...
    private void addResetButton(final JPanel mainPanel) {
        resetButton = new ResetButton(e -> {
            ...
            if (dialogResult == 0) {
                ...
                for (NumberText field : numberFields) {
                    Space space = field.getSpace();
                    if (space.isFixed()) {
                        field.setText(String.valueOf(space.getExpected())); // Restaura valores fixos
                        field.setEnabled(false); // Trava os campos fixos
                    } else {
                        field.setText(""); // Limpa os campos editáveis
                        field.setEnabled(true); // Reativa os campos editáveis
                    }
                }
